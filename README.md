...
搜索正常结束但没有合格候选，与程序异常、输入条件不足、取消和超时分别显示。候选开始、拟合、完成和终止记录持续保存；刷新后可以查看实际尝试数、逐轮原因及已生成的部分结果。历史记录缺失时显示未知，不补成零。

建模准备完成不代表模型已经拟合。已有拟合结果但没有合格赢家时，系统会明确说明，后续正式模型评审不执行；不会制造最佳模型，也不会把初始筛选数据作为胜者数据导出。此时生成的报告是部分诊断报告，不构成生产准入。选择仅使用验证集，没有赢家时不评价测试集。
