...
当前内置钢铁高炉铁水质量预测、钢厂加热炉、火电燃煤锅炉、污水曝气池、水泥回转窑、蒸馏塔、炼油脱丁烷塔和工业干燥器八套字段规范。高炉场景提供带 CC BY 4.0 来源说明的 Mendeley 真实过程数据演示切片，并对非等间隔铁水化验采用只向后匹配的因果对齐；工业干燥器支持10秒采样、3输入3输出、快速动态窗口以及共享输入的多输出ARX模型组。评审结果分为离线候选模型、软测量候选和闭环控制候选；离线指标通过不代表已经具备生产投运条件。
