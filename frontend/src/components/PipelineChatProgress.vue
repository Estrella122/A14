...
import { pipelineStatus } from '../utils/executionStatus'
const props = defineProps({ run: { type: Object, required: true } })
const labels = { partial: '已拟合，无合格赢家', blocked: '条件不足 / 未执行', cancelled: '已取消', timed_out: '已超时', skipped: '未执行', completed: '已完成', failed: '执行失败', needs_review: '待复核', running: '进行中', queued: '排队中', pending: '等待中' }
...
<template>
  <details class="pipeline-chat-progress">
    <summary>数据处理 · {{ pipelineStatus(run) }}<span v-if="current"> · {{ current.label }}</span><small>展开查看实际步骤</small></summary>
