...
const hasWinner = computed(() => report.value?.best_round != null)
...
<template>
  <section v-if="visible" class="run-outcome" aria-label="本次数值任务状态">
    <h2>{{ hasWinner ? `已选出第 ${report.best_round} 轮最佳模型` : labels[report.optimization_outcome] || '寻优结果待核对' }}</h2>
    ...
    <p v-if="hasWinner">按验证综合分选出最佳模型，预测结果、模型指标和报告可查看与导出。质量提示保留，生产适用性以工程评审为准。</p>
    <p v-else>{{ count.evaluated > 0 ? (report.execution_status === 'running' ? '候选拟合已执行，正在比较验证得分。' : '候选拟合已执行，当前没有选中模型。') : '尚未取得有效拟合模型；建模准备完成不代表辨识成功。' }}{{ report.execution_status === 'running' ? '' : ' 正式模型评审未执行；清洗结果与已保存候选可继续查看。' }}</p>
