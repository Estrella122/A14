...
function fitCameraToModel() {
  ...
  const framingDetails = new Set(['maintenance_platform', 'process_piping'])
  ...
  const distance = (maxSize * .68) / Math.tan(THREE.MathUtils.degToRad(camera.fov * .5))
  // Match the source CAD's authored isometric viewpoint instead of looking
  // almost along the dryer drum axis.
  const direction = new THREE.Vector3(1, .4, -1).normalize()
...
function focusNode(nodeId) {
  ...
  controls.target.copy(center)
  camera.position.copy(center.clone().add(direction.multiplyScalar(Math.max(4.5, size * 2.1))))
  controls.update()
}
...
function pointerDown(event) { pointerDownAt = { x: event.clientX, y: event.clientY } }
