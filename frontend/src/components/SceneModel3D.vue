...
const visualRun = computed(() => scene3d.value.source === 'data_scene' ? props.latestRun : null)
const previewRow = computed(() => visualRun.value?.results?.standardization?.preview?.at(-1) ?? {})
const fieldMetadata = computed(() => Object.fromEntries((visualRun.value?.results?.standardization?.mapping?.mappings ?? []).filter((item) => item.standard).map((item) => [item.standard, item])))
  ...
  <section class="scene-shell" :style="{ '--scene-accent': descriptor.accent }">
    <header class="scene-toolbar">
      ...
      <div class="toolbar-actions">
        <span v-if="scene3d.source === 'project_fallback'" class="fallback-state">项目模型回退</span>
