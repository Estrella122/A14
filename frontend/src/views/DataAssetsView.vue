...
function buildFiles(project) {
  ...
  if (project.scenarioId === 'debutanizer_column') return [
    { id: 1, name: 'debutanizer_process_data.csv', source: 'Fortuna 等公开工业基准', rows: 2394, variables: 8, size: '授权后本地导入', period: '有序样本 1 — 2394', quality: null, status: '待授权数据' },
  ]
...
async function runPipelineFile(file, pending = createPendingFile(file)) {
  ...
  try {
    latestRun.value = await uploadPipelineFile(file, { scenarioId: props.project.scenarioId ?? 'auto', instruction: '请根据上传数据识别工业场景并执行APC建模', resampleRule: props.project.resampleRule, maxLag: props.project.maxLag })
