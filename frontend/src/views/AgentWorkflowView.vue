...
async function handleCsv(event) {
  ...
  try {
    const snapshot = await uploadPipelineFile(file, { scenarioId: props.project.scenarioId ?? 'auto', instruction: '请根据上传数据识别工业场景，由Agent总控从头执行并生成分析报告', resampleRule: props.project.resampleRule, maxLag: props.project.maxLag })
