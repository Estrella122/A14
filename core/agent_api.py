...
def agent_trace(request, run_id):
    ...
    def evidence_status(stage_keys=(), *, available=False, terminal=False):
        ...
        if any(value == "failed" for value in states):
            return "failed"
        if any(value in {"partial", "blocked", "cancelled", "timed_out"} for value in states):
            return "partial"
    ...
    nodes = [
        ...
        {"id": "decision", "name": "下一步决策", "kind": "decision", "duration_ms": stage_duration.get("optimization", 0), "status": evidence_status(("optimization", "review"), available=bool(optimization.get("iterations") or review)), "input": {"rounds": optimization.get("candidate_counts", {}).get("attempted", len(optimization["iterations"]) if "iterations" in optimization else None), "best_round": optimization.get("best_round")}, "output": {"action": "deliver" if review.get("passed") else "review", "reason": optimization.get("stop_reason") or optimization.get("stopping", {}).get("stop_reason") or review.get("conclusion") or "暂未取得原因"}},
