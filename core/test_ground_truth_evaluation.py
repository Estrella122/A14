...
ROOT=Path(__file__).resolve().parents[1];P=ROOT/'core/fixtures/regression'


class GroundTruthEvaluationTests(SimpleTestCase):
    def setUp(self):
        self.gt=json.loads((ROOT/'core/fixtures/regression/real_field_ground_truth.json').read_text())
        self.coverage=json.loads((P/'datasets__field_ground_truth__coverage.json').read_text())
    ...
    def test_retrieval_metrics_include_denominators_and_no_training_claim(self):
        m=json.loads((P/'datasets__field_ground_truth__field_metrics.json').read_text())
    ...
    def test_hard_negatives_never_auto_accept(self):
        probes=json.loads((P/'datasets__field_ground_truth__field_metrics.json').read_text())['hard_negative_probes']
        self.assertEqual(4,len(probes));self.assertFalse(any(x['auto_accept'] for x in probes))

    def test_ood_not_confirmed_known(self):
        rows=json.loads((P/'datasets__field_ground_truth__scene_evaluation.json').read_text())
    ...
    def test_alias_split_and_source_dataset_no_leakage(self):
        ...
        manifest=json.loads((P/'datasets__field_ground_truth__dataset_manifest.json').read_text())
    ...
    def test_three_scene_real_contract_training_cannot_fill_missing(self):
        ...
        self.assertEqual([],json.loads((ROOT/'core/fixtures/regression/three_scene_real_runtime.json').read_text())['transformation_policy']['derived_columns'])

    def test_three_scene_pipeline_receipts_are_truthful(self):
        result=json.loads((ROOT/'core/fixtures/regression/three_scene_post_training_runtime.json').read_text())
