...
class RuntimeEventStore:
    ...
    def __init__(self, skill_run_id: str, pipeline_run_id: str | None = None):
        ...
        if not self.state_path.exists():
            self._write_state({"skill_run_id": skill_run_id, "run_id": pipeline_run_id, "status": "running", "last_sequence": 0, "event_count": 0, "payload_bytes": 0, "result": None, "error": None})
...
def get_runtime_event_store(skill_run_id: str) -> RuntimeEventStore | None:
    ...
    return store if store.state_path.exists() else None
